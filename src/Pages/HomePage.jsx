...
const HomePage = () => {
    ...
    const handleShareTravel = async (travelId, tags) => {
        try {
            await axios.put(`https://localhost:7110/api/travels/${travelId}/share`, { tags });
            alert('Путешествие успешно опубликовано!');

            // Обновляем список путешествий
            const response = await axios.get(`https://localhost:7110/api/routes/${user.id}`);
    ...
    const handleDelete = (id) => {
        axios.delete(`https://localhost:7110/api/routes/${id}`)
            .then(() => {
